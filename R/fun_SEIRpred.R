...
SEIRpred <- function(pars, 
  ...
  update_func <- function(stage_pars, states_old) {
    ...
    R <- states_old[9]
    ## new values
    S_new <- S - b * S * (alpha * P +alpha * Ae+alpha *m * Al+ Ie +m*Il ) / N + n - n * S / N
    E_new <- E + b * S * (alpha * P +alpha * Ae+alpha *m * Al+ Ie +m*Il) / N - E / De - n * E /N
    P_new <- P +  E / De  - P / Dp - n * P / N
    Ie_new <- Ie + r * P / Dp - Ie / Die - Ie / Dq
    Il_new<-Il+ Ie / Die - Il/Dil - Il/Dq
    Ae_new <- Ae + (1 - r) * P / Dp - Ae / Die - n * Ae /N
    Al_new<-Al + Ae / Die -Al/Dil - n * Al /N
    H_new <- H + Ie / Dq + Il/Dq - H / Dh
    R_new <- R + H / Dh + (Al + Il) / Dil - n * R / N
    Onset_expect <- r * P / Dp
    expect_H<- Ie / Dq + Il/Dq
    expect_A<-(1 - r) * P / Dp
    ##
    return(c(S_new, E_new, P_new, Ie_new,Il_new, Ae_new,Al_new, H_new, R_new, Onset_expect,expect_H,expect_A))
  }
  ## matrix for results
  states_mat <- matrix(0, length(days_to_fit), length(init_states) + 4)
  ...
  states_mat[,"Onset_expect"]<-states_mat[,"Onset_expect"]*qd###zengjia
  return(states_mat)
}
