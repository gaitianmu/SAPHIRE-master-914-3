...
##
pdf("../output/simulate_resurge_fig3_A-new.pdf", width = 14, height = 7)
...
estI_mat<- t(est_mat[, c("Ie")])+t(est_mat[, c("Il")])
estA_mat<- t(est_mat[, c("Ae")])+t(est_mat[, c("Al")])
estP_mat<- t(est_mat[, c("P")])
#estIAP_mat <- t(est_mat[, c("Ie", "Ae", "P")])


estIAP_mat <- rbind(estI_mat, estA_mat, estP_mat)
...
legend("topleft", legend = c("Presymptomatic (P)", "Unascertainable (A)", "Ascertainable (I)"), fill = c("#0072B5FF", "#FFDC91FF", "#BC3C29FF"), bty = "n")
...
text(barpos[Ieq0_index], 10000, labels = "First day of zero \n new ascertainable cases ", pos = 3)
arrows(barpos[Ieq0_index + 14], 10000, barpos[Ieq0_index+14], barht[Ieq0_index+14], length = 0.1, lwd = 2, col = "blue")
...
arrows(barpos[AIPgt100_index], 10000, barpos[AIPgt100_index], sum(estIAP_mat[, AIPgt100_index]), length = 0.1, lwd = 2, col = "green")
text(barpos[AIPgt100_index-2], 10000, labels = "Resurgence \n defined by I>100", pos = 3)
##
par(fig = c(0.5, 0.965, 0.3, 0.85), new=TRUE)
...
arrows(barpos[11+14], -10000, barpos[11+14], 0, length = 0.1, lwd = 2, xpd = T, col = "blue")
#text(barpos[11+14], -10000, labels = "Lift all controls", pos = 1, xpd = T)
arrows(barpos[length(barpos) - 5], -10000, barpos[length(barpos) - 5], 0, length = 0.1, lwd = 2, xpd = T, col = "green")
